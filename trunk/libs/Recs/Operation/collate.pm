...
sub output {
   ...
   my $output_hash = Recs::Aggregator::map_squish($aggregators, $aggregaotr_values);

   my $keys = $this->{'KEYS'};
   for(my $i = 0; $i < @$keys; ++$i) {
      $output_hash->{$keys->[$i]} = $record_keys->[$i];
   }

   $this->push_record(Recs::Record->new($output_hash));
}
...
sub get_keys {
   ...
   my @keys = map { $record->{$_} } @{$this->{'KEYS'}};
   return \@keys;
}
